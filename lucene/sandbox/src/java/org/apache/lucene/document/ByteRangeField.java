 ...
 * up to a maximum of 4 dimensions (indexed as 8 numeric values). With 1 dimension representing a single byte range,
 ...
 *
 * <p>
 * This field defines the following static factory methods for common search operations over byte ranges:
 * <ul>
...
public class ByteRangeField extends Field {
  /** stores byte values so number of bytes is, well, 1 */
  public static final int BYTES = Byte.BYTES;

  /**
   * Create a new ByteRangeField type, from min/max parallel arrays
   *
   ...
   */
  public ByteRangeField(String name, final byte[] min, final byte[] max) {
  ...
  private static FieldType getType(int dimensions) {
    if (dimensions > 4) {
      throw new IllegalArgumentException("ByteRangeField does not support greater than 4 dimensions");
    }
   ...
   */
  public void setRangeValues(byte[] min, byte[] max) {
    checkArgs(min, max);
   ...
   * @param max
   */
  private static byte[] encode(byte[] min, byte[] max) {
    checkArgs(min, max);
   ...
   */
  static void verifyAndEncode(byte[] min, byte[] max, byte[] bytes) {
  ...
  private static void encode(byte val, byte[] bytes, int offset) {
    byteToSortableBytes(val, bytes, offset);
  }
  ...
  static byte decodeMin(byte[] b, int dimension) {
    int offset = dimension*BYTES;
    return sortableBytesToByte(b, offset);
  }

  /** decodes the max value (for the defined dimension) from the encoded input byte array */
  static byte decodeMax(byte[] b, int dimension) {
   ...
   */
  public static Query newIntersectsQuery(String field, final byte[] min, final byte[] max) {
    return new RangeFieldQuery(field, encode(min, max), min.length, QueryType.INTERSECTS) {
      @Override
      protected String toString(byte[] ranges, int dimension) {
        return ByteRangeField.toString(ranges, dimension);
      }
    };
  }
   ...
   * @throws IllegalArgumentException if {@code field} is null, {@code min} or {@code max} is invalid
   */
  public static Query newContainsQuery(String field, final byte[] min, final byte[] max) {
   ...
   * @throws IllegalArgumentException if {@code field} is null, {@code min} or {@code max} is invalid
   */
  public static Query newWithinQuery(String field, final byte[] min, final byte[] max) {
    checkArgs(min, max);
  ...
  /**
   * These are lifted from {@link org.apache.lucene.util.NumericUtils} and adapted to use on individual bytes
   */

  /**
   * Encodes an integer {@code value} such that unsigned byte order comparison
   * is consistent with {@link Integer#compare(int, int)}
   * @see #sortableBytesToByte(byte[], int)
   */
  public static void byteToSortableBytes(byte value, byte[] result, int offset) {
    // Flip the sign bit, so negative bytes sort before positive bytes correctly:
    value ^= 0x80;
    result[offset] = value;
  }

  /**
   * Decodes a byte value previously written with {@link #byteToSortableBytes}
   * @see #byteToSortableBytes(byte, byte[], int)
   */
  public static byte sortableBytesToByte(byte[] encoded, int offset) {
    byte x = encoded[offset];
