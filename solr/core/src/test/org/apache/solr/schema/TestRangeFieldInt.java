...
public class TestRangeFieldInt extends SolrTestCaseJ4 {
  ...
  public void testRangeQuery() throws Exception {
    ...
    assertU(adoc("id", "0", "range_i", "[0 TO 10, 1 TO 10, 0, 0]"));
    assertU(adoc("id", "1", "range_i", "[1 TO 10, 1 TO 10, 0, 69389]"));
    assertU(adoc("id", "2", "range_i", "[1 TO 10, 1 TO 5, 0, 59389]"));
    assertU(adoc("id", "3", "range_i", "[0,0,0,49389]"));
    assertU(commit());

    SolrQueryRequest req = req("q", "{!pr field=range_i method=intersects}(4, 9, 0, 0)", "fl", "range_i");
    assertQ(req, "//result[@numFound='1']");

    String out = h.query(req);
    assertTrue(out.contains("[0 TO 10"));

    assertQ(req("q", "{!pr field=range_i method=i}[0, 0, 0, 40000 TO 80000]"), "//result[@numFound='1']");
    assertQ(req("q", "{!pr field=range_i method=i}[5, 5, 0, 40000 TO 80000]"), "//result[@numFound='2']");
    assertQ(req("q", "{!pr field=range_i method=i}[5, 5, 0, 40000 TO 60000]"), "//result[@numFound='1']");
    assertQ(req("q", "{!pr field=range_i method=i}[5, 10, 0, 40000 TO 80000]"), "//result[@numFound='1']");
    assertQ(req("q", "{!pr field=range_i method=i}[5, 10, 12, 40000 TO 80000]"), "//result[@numFound='0']");
